...
/*   Updated: 2018/10/15 22:22:43 by hasmith          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void julia(t_mlx *mast, int y){
	...
	mast->re_factor = (mast->maxRe-mast->minRe) / mast->width/ mast->zoom;
	mast->im_factor = (mast->maxIm-mast->minIm) / mast->width/ mast->zoom;
	for(int x=0; x<mast->width; ++x)
	{
		Z_re = mast->minRe + (x * mast->re_factor);
		Z_im = mast->maxIm - (y * mast->im_factor);
